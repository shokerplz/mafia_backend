...
def canv(users,aliveusers,to):               # can vote
    for user in users:
        if ((user['role'] == 'mafia' and user['id'] == to) or (not(aliver(aliveusers,to)))):
            return False
    return True
...
def cank(peacefulusers,users,aliveusers,to): # can kill
    for user in users:
        if ((user['role'] == 'peaceful' and user['id'] == to) and (aliver(aliveusers,to))):
            return True
    return False


def scanpeaceful(users,roomstatus):          # some peaceful alive
    for user in users:
        if((user['role'] == 'peaceful') and (user['id'] in roomstatus['alive'])):
            return True
    return False
...
USERS_IN_ROOM = 6                           # players (if players > 9 - upper sleeptime)
NUMBER_OF_ROUNDS = 40                        # rounds
...
for rnd in range (NUMBER_OF_ROUNDS):
    ...
    if not(scanpeaceful(room_status['users'],room_status)):  # scan for peaceful
        break
